<?php
    ...
    function listone_donhang($id){
        $sql = "SELECT dh.id, dh.soluong, dh.pttt, dh.ngaydathang, dh.status, tk.name as tentk, sp.price as giasp, sp.img, sp.name as tensp, tk.address as diachi, tk.tel FROM donhang dh INNER JOIN taikhoan tk ON dh.iduser = tk.id INNER JOIN sanpham sp ON dh.idpro = sp.id WHERE dh.id = '$id'";
